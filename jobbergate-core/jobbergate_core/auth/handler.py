...
class JobbergateAuthHandler:
    ...
    from both [requests](https://requests.readthedocs.io/en/latest/)  and [httpx](https://www.python-httpx.org/)
    packages by passing it to the `auth` parameter on the request (see examples below).

    It just works out of the box. Behind the scenes, this procedure calls the
    [`acquire_access()`][jobbergate_core.JobbergateAuthHandler.acquire_access] method to load the available tokens from
    the cache directory, it tries to refresh them if they are expired, or provides an URL to the user to login on the
    system.
    ...
    Examples:
        ...
        The following example shows how to use the JobbergateAuthHandler
