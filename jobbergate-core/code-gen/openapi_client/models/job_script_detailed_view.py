...
class JobScriptDetailedView(BaseModel):
    """
    Model to match database for the JobScript resource.  # noqa: E501
    """
    ...
    is_archived: StrictBool = Field(..., description="Indicates if the job script has been archived.")
    ...
    parent_template_id: Optional[StrictInt] = Field(None, description="The foreign-key to the job script template from which this instance was created")
    template: Optional[JobTemplateListView] = None
    cloned_from_id: Optional[StrictInt] = Field(None, description="Indicates the id this entry has been cloned from, if any.")
    files: Optional[conlist(JobScriptFileDetailedView)] = None
    __properties = ["id", "name", "owner_email", "created_at", "updated_at", "is_archived", "description", "parent_template_id", "template", "cloned_from_id", "files"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> JobScriptDetailedView:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> JobScriptDetailedView:
        """Create an instance of JobScriptDetailedView from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return JobScriptDetailedView.parse_obj(obj)

        _obj = JobScriptDetailedView.parse_obj({
            "id": obj.get("id"),
            "name": obj.get("name"),
            ...
            "parent_template_id": obj.get("parent_template_id"),
            "template": JobTemplateListView.from_dict(obj.get("template")) if obj.get("template") is not None else None,
            "cloned_from_id": obj.get("cloned_from_id"),
            "files": [JobScriptFileDetailedView.from_dict(_item) for _item in obj.get("files")] if obj.get("files") is not None else None
        })
        return _obj
