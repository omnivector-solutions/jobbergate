...
### 4.3 Question wire format — JSON Schema

Every question crosses the wire with a standard **JSON Schema** describing its answer. The
schema is generated by the `QuestionBase` classes themselves
(`jobbergate_cli/subapps/applications/questions.py`, `json_schema()`), making it the **single
source of truth**: the same object drives the runtime validators, the front-end widget
auto-generation, and browser-side pre-validation.

```json
{
  "variablename": "ntasks",
  "message": "How many tasks? (1 to 8)",
  "default": 2,
  "schema": {
    "title": "How many tasks? (1 to 8)",
    "type": "integer",
    "minimum": 1,
    "maximum": 8,
    "default": 2
  },
  "type": "Integer",              // legacy hint, kept as a fallback
  "choices": null                 //   (List / Checkbox fallback)
}
```

How each question type maps to a schema:

| Question | Schema fragment |
|---|---|
| `Text` | `{"type": "string"}` |
| `Integer(minval, maxval)` | `{"type": "integer", "minimum": ..., "maximum": ...}` |
| `List(choices)` | `{"enum": [...]}` |
| `Checkbox(choices)` | `{"type": "array", "items": {"enum": [...]}, "uniqueItems": true}` |
| `Confirm` / `BooleanList` | `{"type": "boolean"}` |
| `File` / `Directory` | `{"type": "string", "x-path-type": ..., "x-exists": ...}` |
| `Const` | `{"const": <default>}` |

SMEs can also pass extra constraints straight into any question —
`Text("email", ..., schema={"pattern": r"^\S+@\S+\.\S+$"})` — and they are merged over the
generated fragment **and enforced as a validator** wherever the question is asked (terminal
`inquirer` flow and web runner alike, via `jsonschema`).

Notes:

- The web form **pre-validates against the schema in the browser** (type, range, enum,
  pattern, length) for instant feedback with no round-trip. The runtime's verdict on
  `POST /answers` remains **authoritative**: it re-runs the schema check plus everything the
  schema cannot express — imperative validators and, meaningfully, `File`/`Directory`
  existence against the *cluster* filesystem (`x-exists` is only a hint to the UI).
...
- `QuestionBase` and its subclasses generate a **JSON Schema** per question (`json_schema()`,
  see §4.3) and accept SME-supplied `schema=` constraints, enforced via `jsonschema` in every
  prompting backend — the schema is the single source of truth for wire format, widgets, and
  validation.
- `subapps/applications/remote_prompt.py::BridgePrompter` serves prompts one at a time over the
  bridge (each carrying its schema) and validates each answer with the original `inquirer`
  validators plus the schema.
