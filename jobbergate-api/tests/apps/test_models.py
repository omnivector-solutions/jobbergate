...
@pytest.mark.parametrize("model_cls", (JobScriptTemplate, JobScript, JobSubmission))
def test_searchable_fields_are_strings(model_cls):
    """Test that the searchable fields are strings, so search will work."""
    assert all(isinstance(c.type, String) for c in model_cls.searchable_fields())
