...
_ENVIRONMENT_FLAG = "EXPORT"

_INLINE_COMMENT_MARK = "#"


def slurm_flagged_line(line: str) -> bool:
    """
    Identify if a provided line starts with the identification flag.
    """
    return line.upper().startswith(_IDENTIFICATION_FLAG)

def env_var_flagged_line(line: str) -> bool:
    """
    Identify if a provided line starts with the identification flag.
    """
    return line.upper().startswith(_ENVIRONMENT_FLAG)


def _clean_line_sbatch(line: str) -> str:
    """
...
def _clean_line_env_var(line: str) -> str:
    """
    Clean the provided line.
    ...
    then remove the inline comment mark and anything after it, and finally strip white spaces at both sides.
    """
    return line.upper().lstrip(_ENVIRONMENT_FLAG).split(_INLINE_COMMENT_MARK)[0].strip()


def _clean_jobscript(jobscript: str, indentification_flag=_IDENTIFICATION_FLAG) -> List[str]:
    """
    ...
    in a single list.
    """
    # TODO - create a parameter for identification flag
    if indentification_flag == _IDENTIFICATION_FLAG:
        jobscript_filtered = filter(slurm_flagged_line, jobscript.splitlines())
        jobscript_cleaned = map(_clean_line_sbatch, jobscript_filtered)
    elif indentification_flag == _ENVIRONMENT_FLAG:
        jobscript_filtered = filter(env_var_flagged_line, jobscript.splitlines())
        jobscript_cleaned = map(_clean_line_env_var, jobscript_filtered)
...
# TODO - Complete the values of Slurm environment variables from slurm documentation at 
# https://slurm.schedmd.com/sbatch.html
sbatch_to_slurm_mapping = [
    SbatchToSlurm("account", "--account", "-A", "SBATCH_ACCOUNT"),
    SbatchToSlurm("account_gather_frequency", "--acctg-freq", "", "SBATCH_ACCTG_FREQ"),
    SbatchToSlurm("array", "--array", "-a", "SBATCH_ARRAY_INX"),
    ...
    SbatchToSlurm("cpus_per_gpu", "--cpus-per-gpu", "", "SBATCH_CPUS_PER_GPU"),
    SbatchToSlurm("cpus_per_task", "--cpus-per-task", "-c", "SLURM_CPUS_PER_TASK", dict(type=int)),
...
def jobscript_to_dict(jobscript: str) -> Dict[str, Union[str, bool]]:
    ...
    """
    sbatch_args_script = _clean_jobscript(jobscript, _IDENTIFICATION_FLAG)
    env_vars_args = _clean_jobscript(jobscript, _ENVIRONMENT_FLAG)

    parsed_args_sbatch, unknown_arg_sbatch = parser.parse_known_args(args=sbatch_args_script)
    parsed_args_env_vars, unknown_arg_env_vars = parser.parse_known_args(args=env_vars_args)

    if unknown_arg_sbatch:
        raise ValueError("Unrecognized SBATCH arguments: {}".format(" ".join(unknown_arg_sbatch)))
    
    sbatch_params = {key: value for key, value in vars(parsed_args_sbatch).items() if value is not None}
    logger.debug(f"SBATCH params parsed from job script: {sbatch_params}")

    env_var_params = {key: value for key, value in vars(parsed_args_env_vars).items() if value is not None}
    logger.debug(f"Enviromnet variables params parsed from job script: {env_var_params}")

    return {**sbatch_params, **env_var_params}
