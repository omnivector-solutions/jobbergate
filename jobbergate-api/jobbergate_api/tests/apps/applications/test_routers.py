...
async def test_get_application_all_from_user_empty(client, application_data, inject_security_header):
    ...
    """
    applications = [
        Application(id=1, application_owner_email=999, **application_data),
        Application(id=2, application_owner_email=999, **application_data),
        Application(id=3, application_owner_email=999, **application_data),
    ]
    await insert_objects(applications, applications_table)
    count = await database.fetch_all("SELECT COUNT(*) FROM applications")
    assert count[0][0] == 3
...
async def test_get_all_applications_pagination(client, application_data, inject_security_header):
    ...
    applications = [
        Application(id=1, application_owner_email="owner1@org.com", **application_data),
        Application(id=2, application_owner_email="owner1@org.com", **application_data),
        Application(id=3, application_owner_email="owner1@org.com", **application_data),
    ]
    await insert_objects(applications, applications_table)
    count = await database.fetch_all("SELECT COUNT(*) FROM applications")
    assert count[0][0] == 3
